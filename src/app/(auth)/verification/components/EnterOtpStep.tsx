...
import { useVerify, useResendCode } from "@/features/auth/hooks/useVerify";
import { display } from "@mui/system";
import { PhoneIcon, QuestionMarkIcon, TimerIcon2 } from "@/iconsComponents/all";
// import SuccessCard from "./SuccessCard";
import { useRouter } from "next/navigation";
import { useContext } from "react";
import { CircularProgress } from "@mui/material";
import { setEmail, setVerificationError, setTimeLeft, setSource, setStep, setVerifiedUser } from "@/Redux/slices/otpVerificationSlice";
...
export default function EnterOtpStep() {
  ...
  const { email, verificationErrorState: verificationError, verificationTimer: timeLeft, source } = useAppSelector(state => state.otpVerification)
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => dispatch(setTimeLeft(Number(timeLeft) - 1)), 1000);
      ...
      >
        ...
        <Stack width={"100%"} mx={"auto"}>
          ...
          {!verificationError ? (
            ...
            (
              <Stack>
                <Typography mt={"10px"} sx={{ color: "#EF4444", fontSize: "12px", fontWeight: "400", lineHeight: "20px" }}>
                ...
                <Stack flexDirection={"row"} alignItems={"center"} gap={"4px"} justifyContent={"center"}>
                  <Typography sx={{ color: "#EF4444", fontSize: "12px", fontWeight: "400", lineHeight: "20px" }}>
                    You can resend after
                  </Typography>
                  <Stack flexDirection={"row"} gap={"2px"} alignItems={"center"}>
                    <TimerIcon2 color="#EF4444" />
                    <Typography sx={{ fontSize: "12px", fontWeight: 500, marginTop: "2px", color: "#EF4444" }}>
                      {formattedTime}
                    </Typography>
                  </Stack>
                </Stack>
