import { getNotifications, markAsRead, markAllAsRead, clearAll,getUnreadNotificationsCount,remove } from "../services";
        ...
        onMutate:async (id)=>{
          await queryClient.cancelQueries({ queryKey: ["notifications"] })
            ...
            queryClient.setQueryData(["notifications"],(oldData:any)=>{
               const updated = {...oldData,data:oldData?.data?.map((item:NotificationType)=>{
                if (item.id===id) {
                    return {
                        ...item,
                        isRead:true
        ...
        mutationFn: () => markAllAsRead(),
         onMutate:async (id)=>{
          await queryClient.cancelQueries({ queryKey: ["notifications"] })
            const prev = queryClient.getQueryData(["notifications"]) 
            queryClient.setQueryData(["notifications"],(oldData:any)=>{
               return {...oldData,data:oldData?.data?.map((item:NotificationType)=>{
                    return {
                        ...item,
                        isRead:true
                    }
        ...
        mutationFn: (id: string | number) =>remove(id),
          onMutate:async (id)=>{
           await queryClient.cancelQueries({ queryKey: ["notifications"] })
            ...
            queryClient.setQueryData(["notifications"],(oldData:any)=>{
                if (!oldData) return oldData
               return {...oldData,data:oldData?.data?.filter((item:NotificationType)=>item.id!==id)
}
            })
            return {prev}
        },
