...
import SizeFilterBottomSheet from "../filters/bottomSheetFilters/size";
...
export default function SmallScreenFilterationComponent({ open, setOpen, categories, allColors, sizes }: { open: boolean, setOpen: (open: boolean) => void, categories: mainCategoryType[], allColors: colorType[], sizes: sizeType[] }) {
    const t = useTheme()
    ...
    return (
        <BottomSheet open={open} setOpen={setOpen} >
            <Stack bgcolor={t.tokens.backgroundColors.main} direction={"column"}>
                <Stack height={"60vh"} overflow={"hidden"} padding={"16px"} >
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid size={8} maxHeight={"60vh"} sx={{ overflowY: "auto" }}>
                            {activeFilter === "Category" && <CategoryFilterBottomSheet categories={categories} />}
                            {activeFilter === "Stock Status" && <StockFilterBottomSheet />}
                            {activeFilter === "Size" && <SizeFilterBottomSheet sizes={sizes} />}
                            {activeFilter === "Color" && <ColorFilterBottomSheet allColors={allColors} />}
